...
static const int GPIO_SCLK = 12;
// static const int GPIO_MISO = 13;
// static const int GPIO_MOSI = 11;
...
void spi_master_init(VS1053_t * dev, int16_t GPIO_CS, int16_t GPIO_DCS, int16_t GPIO_DREQ, int16_t GPIO_RESET)
{
	...
	ret = spi_bus_initialize( 2, &buscfg, 3 );
	assert(ret==ESP_OK);
	...
	ret = spi_bus_add_device( 2, &devcfg, &lvsspi);
	...
	if (testComm(dev, "Slow SPI,Testing VS1053 read/write registers...\n")) {
		...
		write_register(dev, SCI_CLOCKF, 7 << 13); // Normal clock settings multiplyer 3.0 = 12.2 MHz
		...
		ret = spi_bus_add_device( 2, &devcfg, &hvsspi);
		ESP_LOGD(TAG, "spi_bus_add_device=%d",ret);
		assert(ret==ESP_OK);
